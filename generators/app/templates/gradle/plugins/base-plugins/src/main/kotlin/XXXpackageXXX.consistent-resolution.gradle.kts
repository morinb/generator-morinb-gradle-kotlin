plugins {
    id("java")
    id("<%= package %>.dependency-analysis-project")
}
...
val appRuntimeClasspath = configurations.create("appRuntimeClasspath") {
    ...
    withDependencies {
        ...
        add(project.dependencies.create(project.dependencies.platform("<%= package %>.product:platform")))
    }
}
...
sourceSets.all {
    ...
    if (this != sourceSets.main.get()) {
        dependencies.add(implementationConfigurationName, dependencies.platform("<%= package %>.product:platform"))
    }
}
